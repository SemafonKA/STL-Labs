...
//void dijkstra(const vector < vector < pii > >& g, int f, vector < int > &distance, vector < int > &parent)
//{
...
//         continue;
//
//      out << getPath(distance, parent, startVertex, i);
//   }
//   out.close();
//}
//
...
int main()
{
   ...
   vector < vector < pii > > gr(n + 1);  // [first]: vertexTo, [second]: weight 
   ...
   timer.reset();
#pragma omp parallel for num_threads(12) schedule(dynamic, 10)        // more threads == faster
   for (int i = 1; i < gr.size(); ++i)
   {
      getAllRoutesBy(i, gr, out);
   }
